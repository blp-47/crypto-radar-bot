...
class YahooFinanceClient:
    ...
    def quotes(self, symbols: List[str]) -> List[QuoteData]:
        ...
        # Important : on demande explicitement les champs preMarket/postMarket.
        # Sinon Yahoo peut renvoyer seulement le dernier prix régulier, ce qui
        # ferait rater la hausse entre la clôture de la veille et l'ouverture.
        fields = ",".join([
            "symbol",
            "currency",
        ...
        for item in results:
            ...
            # En pré-marché, on privilégie preMarketPrice pour être alerté avant 15h30.
            # En marché ouvert, on prend regularMarketPrice.
            # En after-hours, on prend postMarketPrice, même si allow_afterhours est désactivé par défaut.
            if market_state in {"PRE", "PREPRE"}:
                price = first_float(item, ["preMarketPrice", "regularMarketPrice"])
                ...
                price = first_float(item, ["postMarketPrice", "regularMarketPrice"])
